...
$batContent += "    echo.`r`n"
$batContent += "    `"%PYTHON_EXE%`" -s `"%BASE_DIR%SD-Trainer\setup_environment.py`"`r`n"
$batContent += "    if errorlevel 1 (`r`n"
$batContent += "        echo.`r`n"
$batContent += "        echo  Setup failed. Please check network and run again.`r`n"
$batContent += "        pause`r`n"
$batContent += "        exit /b 1`r`n"
$batContent += "    )`r`n"
$batContent += ")`r`n"
$batContent += "`r`n"
$batContent += ":: Set working directory to project root (so frontend/dist, config/ etc. resolve correctly)`r`n"
$batContent += "cd /d `"%BASE_DIR%SD-Trainer`"`r`n"
$batContent += "`"%PYTHON_EXE%`" -s gui.py --skip-prepare-environment --port 28000`r`n"
$batContent += "pause`r`n"
