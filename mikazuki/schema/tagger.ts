Schema.intersect([
    Schema.object({
        ...
        download_endpoint: Schema.union(["默认", "https://hf-mirror.com", "https://modelscope.cn"]).default("默认").description("模型下载加速源（国内网络建议启用）"),
        path: Schema.string().role('filepicker', { type: "folder" }).required().description("图片文件夹路径"),
