...
def do_sample(
    ...
    for i in tqdm(range(steps), desc="Sampling"):
        ...
        if use_cfg:
            ...
            pos_out_f = pos_out.float()
            del pos_out
            gc.collect()
            # Second forward allocates another full activation footprint; clear cache between passes to reduce OOM / fragmentation spikes.
            if device.type == "cuda":
                torch.cuda.empty_cache()
            neg_out = dit(x, t, neg_crossattn_emb, padding_mask=padding_mask)
            neg_out_f = neg_out.float()
            del neg_out
            gc.collect()

            model_output = neg_out_f + guidance_scale * (pos_out_f - neg_out_f)
            del pos_out_f, neg_out_f
        else:
