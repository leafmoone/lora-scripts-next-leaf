...
class Anima(nn.Module):
    ...
    def forward_mini_train_dit(
        ...
        # IMPORTANT: don't use next(self.parameters()) — with block swapping, early params may live on CPU.
        tdev = self.t_embedder[1].linear_1.weight.device
        tdtype = self.t_embedder[1].linear_1.weight.dtype
        timesteps_B_T = timesteps_B_T.to(device=tdev, dtype=tdtype)
        crossattn_emb = crossattn_emb.to(device=tdev, dtype=crossattn_emb.dtype)
