# Archived Anima LoRA training script
#
# This file is retained only as a migration reference for the pre-wrapper local
# implementation. It is not an active trainer entrypoint. New Anima training must
# go through scripts/dev/anima_train_network.py, which delegates to the pinned
# kohya-ss/sd-scripts backend configured in config/anima_backend.toml.
...
class AnimaNetworkTrainer(train_network.NetworkTrainer):
    ...
    def assert_extra_args(
        ...
        if args.cache_text_encoder_outputs:
            try:
                is_cacheable = train_dataset_group.is_text_encoder_output_cacheable(cache_supports_dropout=True)
            except TypeError:
                is_cacheable = train_dataset_group.is_text_encoder_output_cacheable()
            assert is_cacheable, "when caching Text Encoder output, shuffle_caption, token_warmup_step or caption_tag_dropout_rate cannot be used"
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        ...
        return "anima", text_encoders, vae, unet
